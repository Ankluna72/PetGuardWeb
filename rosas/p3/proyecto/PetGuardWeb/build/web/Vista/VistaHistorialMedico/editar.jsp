...
<div class="historial-layout">
    ...
    <% if (observaciones != null && !observaciones.isEmpty()) {
        for(Observacion o : observaciones) { 
            String autor = "Propietario";
            String autorClase = "autor-propietario";
            String icono = "🐾";
    %>
        <div class="comentario <%= autorClase %>">
            <%-- Solo el propietario puede editar sus observaciones --%>
            <% if (
                "propietario".equals(rol) &&
                obsEditId != null && obsEditId.equals(o.getId().toString()) &&
                dni.equals(o.getDni_propietario())
            ) { %>
                <!-- Formulario de edición de observación SOLO SI la hizo este propietario -->
                <form class="add-comentario-form" action="/PetGuardWeb/ControladorObservacion" method="post">
                    ...
                    <input type="hidden" name="dni_propietario" value="<%= dni %>"/>
            ...
            <% } else { %>
                ...
                <div class="comentario-btns">
                    <%-- Solo el propietario puede editar/eliminar sus observaciones --%>
                    <% if (
                        "propietario".equals(rol) &&
                        dni.equals(o.getDni_propietario())
                    ) { %>
    ...
    <% } %>
    <%-- Solo el propietario puede agregar observaciones --%>
    ...
    <main class="form-panel">
        ...
        <%-- Imagen de la mascota centrada debajo del título --%>
        <%
            String fotoMascota = "/PetGuardWeb/img/mascotasinfoto.png";
            if (mascota != null && mascota.getId() > 0) {
                String urlFoto = mascotaDAO.obtenerFotoPorId(mascota.getId());
        ...
        <div style="display: flex; justify-content: center; margin: 10px 0 10px 0;">
            <img src="<%= fotoMascota %>" alt="Foto de la mascota"
                 style="max-width: 150px; border: 2px solid #aaa; background: #fff;">
        </div>
